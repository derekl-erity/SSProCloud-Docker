...
- Edit variables in `.env.local` as required (start from `.env.example`).
...
- Bring container up in Admin mode `ADMIN=yes docker compose --env-file .env.local up`.
...
- Bring container up normally `docker compose --env-file .env.local up` (`ADMIN=no` is default).
...
Certificate is volume-mounted to the appropriate location in Pro Cloud Server installation. Edit the location in `.env.local` to specify the source.
...
**WebClient will not have access to Pro Cloud Server with the default White List setup.** Add `${BASE_IP}.3` to the White List for the `ssprocloudwebconfig` service.


### Environment File Usage

Use an untracked env file for runtime configuration:
- `cp .env.example .env.local`
- Edit `.env.local` for your deployment
- Run compose with `docker compose --env-file .env.local ...`

### Windows Checkout Note

This repository enforces LF line endings for shell scripts. Validate before running:
```sh
...
- Keep `PCS_BIND_IP` and `WEB_BIND_IP` set to `127.0.0.1` unless deliberate external exposure is required.
- Use a private `BASE_IP` network prefix (default is `172.22.0`).
- For external access, front services with a reverse proxy/TLS terminator instead of direct host-wide binds.
- If WebConfig is exposed outside localhost, switch `webconfig/settings.php` to HTTPS and enable cert enforcement.
- Pin images by digest in production deployments for deterministic pulls.
